...
<h2>🏗 Architecture</h2> <pre> src/ ├── controllers/ ├── routes/ ├── services/ ├── pipeline/ ├── utils/ ├── server.js assets/ ├── survey_form.jpg .env package.json </pre> <h3>Pipeline Flow</h3> <pre> Input → OCR/Text Parsing → Normalization → Factor Extraction → Confidence & Guardrails → Risk Scoring → Recommendations </pre>
